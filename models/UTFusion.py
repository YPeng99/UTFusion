...
class UTFusion(nn.Module):
    ...
    def __init__(self, in_ch=1, n_head=8, patch_size=8):  # 8 8
        super().__init__()
        self.in_ch = in_ch
        self.n_head = n_head
        self.patch_size = patch_size
        self.dim = int(in_ch * patch_size ** 2)
        self.patch = nn.Embedding(2, self.dim)

        self.in_proj = nn.Linear(self.dim,self.dim)
        self.formers = nn.ModuleList([Former(self.dim, self.n_head, self.dim * 2) for _ in range(6)])
    ...
    def forward(self, *input):
        ...
        if len(input) ==1:
            fuse_scheme = torch.ones(outs.shape[0],dtype=torch.int).to(outs.device)
        else:
            fuse_scheme = input[1].repeat(outs.shape[0]) if len(input[1]) == 1 else input[1]

        patch = self.patch(fuse_scheme)
